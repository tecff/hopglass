define(["map/clientlayer", "map/labelslayer",
  ...
  function (ClientLayer, LabelsLayer, d3, L, moment, LocationMarker, rbush) {
    ...
    var iconOnline  = { color: "#1566A9", fillColor: "#1566A9", radius: 5, fillOpacity: 0.5, opacity: 0.5, weight: 2, className: "stroke-first" }
    var iconOffline = { color: "#D43E2A", fillColor: "#D43E2A", radius: 2, fillOpacity: 0.4, opacity: 0.4, weight: 1, className: "stroke-first" }
    var iconLost    = { color: "#D43E2A", fillColor: "#D43E2A", radius: 3, fillOpacity: 0.5, opacity: 0.5, weight: 1, className: "stroke-first" }
      ...
      self.setData = function (data) {
        ...
        var markersLost = data.nodes.lost.filter(has_location)
          .map(mkMarker(nodeDict, function (d) {
            if (d.lastseen.isAfter(moment(data.now).subtract(config.maxAlertHours, "hours")))
              return iconAlert
