define(["moment", "numeral", "tablesort", "tablesort.numeric"],
  ...
  function showGeoURI(d) {
    ...
    return function (el) {
      if (has_location(d)) {
        ...
        el.classList.add("missing-location")
        el.textContent = "kein Standort hinterlegt"
      }
    }
  }
  ...
  function showContact(d) {
    return function (el) {
      var contactInfo = dictGet(d.nodeinfo, ["owner", "contact"])
      if (contactInfo)
        el.textContent = contactInfo
      else {
        el.classList.add("missing-contact")
        el.textContent = "keine Kontaktinformation hinterlegt"
      }
    }
  }
  ...
  function showGateway(d, router, nodeDict) {
    ...
    if (!gw) return null
    return function (el) {
      var nhid = nodeToId(nh, nodeDict)
      var gwid = nodeToId(gw, nodeDict)
      if (nh) {
  ...
  return function(config, el, router, d, nodeDict) {
    ...
    if (config.showContact)
      attributeEntry(attributes, "Kontakt", showContact(d))
